...
def run_tile(x, y, tilename, outputdir = 's1_filtered_geomedian', bands=['vv','vh'], year='2018', redo = False):
    if not os.path.exists(outputdir): os.mkdir(outputdir)
    if len(bands)==1:
        outpath = os.path.join(outputdir,'S1_%s_%s_%s.nc'%(year, bands[0], tilename.replace(',','_')))
    else:
    ...
    if len(bands)==1:
        data = data.where(data[bands[0]]>0.001) # remove noisy values
        ...
        data = data.where(data.vh>0.001) # remove noisy values
...
for index in shpfile.index.values:
    #if not shpfile.loc[index]['label'][0] in '-10': continue
    ...
    #if not shpfile.loc[index]['label'] == '-1,-23': continue
